...
const AutomationSettings = () => {
  ...
  const handleRestore = async (item) => {
    try {
      await restoreAutomationConfig(item._id);
  ...
  const columns = [
    ...
    { key: 'value', label: 'Value', render: (val) => <span style={{ fontSize: 'var(--text-sm)', color: 'var(--color-body)' }}>{typeof val === 'object' ? JSON.stringify(val) : String(val || '')}</span> },
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ position: 'relative', flex: 1, minWidth: '200px' }}>
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search automation configs..." style={{ width: '100%', padding: '0.625rem 1rem 0.625rem 2.25rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--color-border)', background: 'var(--color-card)', fontSize: 'var(--text-base)', outline: 'none' }} />
        </div>
      ...
      <ConfigTable columns={columns} data={configs} loading={isLoading} onEdit={openEdit} onDelete={handleDelete} onToggle={handleToggle} onRestore={handleRestore} emptyTitle="No automation configs" emptyDescription="Enable automation to streamline contribution workflows." />
      ...
      <ConfigModal open={modalOpen} onClose={() => setModalOpen(false)} title={editingItem ? 'Edit Automation Config' : 'Add Automation Config'} loading={loading} footer={
        ...
        <div style={{ marginBottom: '1rem' }}>
          ...
          <input type="text" value={form.key || ''} onChange={(e) => setForm({ ...form, key: e.target.value })} placeholder="e.g. auto_coin_award" style={{ width: '100%', padding: '0.625rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--color-border)', background: 'var(--color-card)', fontSize: 'var(--text-base)', outline: 'none' }} />
        </div>
        <div style={{ marginBottom: '1rem' }}>
        ...
        <div style={{ marginBottom: '1rem' }}>
          ...
          <input type="text" value={form.description || ''} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Describe this automation..." style={{ width: '100%', padding: '0.625rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--color-border)', background: 'var(--color-card)', fontSize: 'var(--text-base)', outline: 'none' }} />
        </div>
      </ConfigModal>
    </div>
  );
